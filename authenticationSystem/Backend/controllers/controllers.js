    ...
    if (checkEmail) {
      ...
      if (checkUser) {
        ...
        if (result) {
          ...
          resp.send({
            message: "data inserted successfully and cookie set",
            userLogged: true,
          });
        } else {
...
//controller function for protected route
const frontendLogginCheck = (req, resp) => {
    resp.status(200).json({userLogged:true,message:req.errorMessage})
};
export { LoginSystem ,frontendLogginCheck};
